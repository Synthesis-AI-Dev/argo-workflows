...
var GetSemaphoreQueTypeAlwaysRebalanced = func(s string) (config.SemaphoreStrategy, error) {
...
func TestSemaphoreTmplLevel(t *testing.T) {
	...
	t.Run("TemplateLevelRebalanced", func(t *testing.T) {
		concurrenyMgr := NewLockManager(syncLimitFunc, GetSemaphoreQueTypeAlwaysRebalanced, func(key string) {
		}, WorkflowExistenceFunc)
		...
		assert.NoError(t, err)
		assert.Equal(t, msg, "Waiting for default/ConfigMap/my-config/rebalancedsem lock. Lock status: 0/2 ")
		assert.False(t, status)
		assert.True(t, wfUpdate)
		assert.NotNil(t, wfRebalanceKeyA.Status.Synchronization)
		assert.NotNil(t, wfRebalanceKeyA.Status.Synchronization.Semaphore)
